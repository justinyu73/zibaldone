...
- **No-key ladder.** Everything works without a paid API key: YouTube captions,
  local Whisper ASR (3 quality tiers), and local LLM summarization via Ollama — or a
  built-in llama.cpp runtime downloaded on first use (~15 MB engine + one model).
  Cloud models are an opt-in upgrade, not a requirement.
...
- **Honest engineering.** 77 API endpoints locked by a surface-contract test,
  340+ backend tests, 21 Playwright E2E cases, release gated on CI, and a size
...
1. **Local, zero-config** — if [Ollama](https://ollama.com) is running, its models
   appear in the picker automatically. The first-run wizard can pull a recommended
   model with one click.
2. **Built-in runtime** — no Ollama? The wizard offers a bundled
   [llama.cpp](https://github.com/ggml-org/llama.cpp) server (downloaded on first
   use, pinned release, checksummed) so a clean machine still gets offline AI.
3. **Subscription CLIs** *(off by default)* — detects logged-in `claude` / `codex` /
   `gemini` CLIs and can route summarization through your existing subscription.
   Disabled unless you explicitly enable it in Settings (calling vendor CLIs
   programmatically may sit in a gray zone of their terms — read them first).
4. **Cloud APIs** — OpenAI / Anthropic / Google keys, stored locally, never logged,
...
- **Provider abstraction** ([`providers.py`](backend/providers.py)): one
  `chat_complete()` across OpenAI/Anthropic/Google/Ollama/llama.cpp/CLI, with
...
.venv/bin/python -m unittest discover tests        # 340+ tests
