...
test('article paste-mode closed loop: save → articles folder → inbox pickup', async ({ page }) => {
  ...
  const sourceLinks = library.getByRole('navigation', { name: '價值庫來源捷徑' })
  await expect(sourceLinks).toContainText('articles')
  await sourceLinks.getByRole('button', { name: /articles\s+1/ }).click()
  await expect(sourceLinks.getByRole('button', { name: /articles\s+1/ })).toHaveAttribute('aria-pressed', 'true')
  await expect(library.getByLabel('價值庫來源分類')).toHaveValue('articles')
