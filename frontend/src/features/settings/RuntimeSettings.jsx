    ...
    <section className="panel settings-panel cost-panel">
      ...
      {runtime ? (
        <div className="settings-form">
          <div className={`settings-state cli-model-notice ${cliEnabled && cliModels.length ? 'ok' : 'info'}`}>
            <Terminal size={15} />
            <span>
              <strong>訂閱 CLI 模型：{cliEnabled ? `已啟用（${cliModels.length} 個可用）` : '目前未啟用'}</strong>
              {cliEnabled
                ? (cliModels.length
          ...
          <label className="settings-toggle">
            ...
            <span>
              ...
              <small>勾選後按「儲存模型/上限」；只會使用你已安裝、已登入的 CLI 與其訂閱額度，app 端不收 API 費用。請先確認各家服務條款。</small>
            </span>
          </label>
