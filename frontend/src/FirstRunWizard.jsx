            ...
            <div className="setup-form">
              {!backendReady || (llmState === 'loading' && !llm) ? (
                ...
                <>
                  ...
                  <button onClick={skipBuiltinDownload}><ArrowRight size={16} />跳過，本次不下載</button>
                ...
                <>
                  ...
                  <div className="settings-note">這裡下載的是內建 llama.cpp 執行引擎與 Gemma 模型，不是 Ollama；訂閱 CLI 也不由本程式下載。</div>
                  <button onClick={skipBuiltinDownload}><ArrowRight size={16} />跳過，本次不下載</button>
                </>
              ) : (
                <>
                  {llm?.builtin?.download?.status === 'error' && <div className="settings-state error">下載失敗：{llm.builtin.download.error}</div>}
                  <div className="settings-state info"><strong>本機 AI（非 Ollama）</strong>：下載內建 llama.cpp 執行引擎＋Gemma 模型後，翻譯與 AI 摘要可離線、免金鑰使用。</div>
                  <div className="settings-note"><strong>訂閱 CLI 不下載：</strong>Claude／Codex／Gemini CLI 必須由你自行安裝並登入；Zibaldone 只會在設定頁偵測並讓你選用。</div>
              ...
              {llmSkipped && <div className="settings-state neutral">本次不下載內建本機 AI；可完成設定，之後在設定頁再下載。</div>}
            </div>
          )}
