...
# resulting bundle is self-contained (ships the FastAPI sidecar onedir tree; no
# dev server / source / venv needed at install time).
#
...
NO_STRIP=1 APPIMAGE_EXTRACT_AND_RUN=1 npx tauri build --bundles deb,appimage \
  --config '{"bundle":{"externalBin":[],"resources":{"binaries/video-intake-fastapi-sidecar":"sidecar"}}}'
