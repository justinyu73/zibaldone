...
fn spawn_sidecar(app: &tauri::AppHandle, state: Arc<SidecarState>, session: Arc<SidecarSession>) {
    ...
    // The packaged PyInstaller sidecar is a console-subsystem executable on
    // Windows. CREATE_NO_WINDOW keeps its stdout/stderr pipes available for
    // readiness/log forwarding without flashing a terminal on every app launch.
