...
def app_state_agent_index_status(vault_root: VaultPath):
    ...
    try:
        return agent_index_status(vault_root)
    except AgentIndexError as exc:
        raise HTTPException(400, str(exc)) from exc


@router.post("/api/app/agent-index")
def app_state_agent_index(req: AgentIndexReq):
    """Generate the explicit, local-only vault→agent index projection."""
    from agent_index import AgentIndexError, generate_agent_index

    try:
        return generate_agent_index(
            req.vault_root,
            write=not req.dry_run,
            confirm=req.confirm,
        )
    except AgentIndexError as exc:
        raise HTTPException(400, str(exc)) from exc
    except OSError as exc:
        raise HTTPException(500, f"寫入 Agent 索引失敗：{exc}") from exc
