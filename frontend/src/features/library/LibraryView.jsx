  ...
  async function search(sortArg, foldersArg) {
    ...
    try {
      if (valueMode && query.trim() && paths.root) {
        ...
        const srcs = Object.entries(data.sources || {}).map(([s, n]) => `${s} ${n}`).join('｜')
        setStatus({ type: 'ok', message: `價值庫 ${data.total || 0} 筆（${srcs}）` })
      } else {
    ...
    <div className="workbench library-workbench">
      <section className="command-bar library-command" aria-label="library search">
        <div className="command-main">
          ...
          <div className="command-title">{selectedTitle}</div>
      ...
      <div className="library-frame">
        ...
        <div className="library-toolbar">
          ...
          {valueMode ? (
            <>
              <label className="library-source-filter">來源分類
                <select aria-label="價值庫來源分類" value={sourceFilter} onChange={(event) => { setSourceFilter(event.target.value); setSelected(null) }}>
