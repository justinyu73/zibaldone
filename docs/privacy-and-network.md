...
The Agent Bridge follows the same boundary: its manual vault index operation
reads Markdown metadata and local links only, writes the derived files under
`_zibaldone/agent-index/`, and makes no provider, connector, telemetry, or
network call. The original vault notes are never uploaded or rewritten by this
operation.
